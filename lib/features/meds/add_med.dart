...
class _AddMedPageState extends State<AddMedPage> {
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty || doseAmt == null || unit.isEmpty || times.isEmpty || obtained == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Fill name, dosage+unit, date obtained, and at least one time.')),
      );
      return;
    }
    ...
    try {
      final inserted = await supa.from('medications').insert({
        ...
        // store as ISO date (no time) to match DATE column
        'date_obtained':
            '${obtained!.year}-${obtained!.month.toString().padLeft(2, '0')}-${obtained!.day.toString().padLeft(2, '0')}',
        'times_local': times,
      }).select().single();
      ...
      // Supabase (RLS/validation) error message:
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Supabase error: ${e.message}')));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Unexpected: $e')));
    } finally {
      ...
      body: ListView(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: saving ? const CircularProgressIndicator(color: Colors.white) : const Text('Save'),
          ),
        ],
      ),
    );
